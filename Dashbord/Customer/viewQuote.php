 ...
 <section class="content">
           ...
           <div class="box-body" style="overflow-x:auto;">
              <table id="quotationitems" class="table-bordered table-hover" width="100%" >
                ...
                <?php
				   //loding the quotation items to the interface
					$query2="SELECT * FROM quotation_item WHERE q_no='".$row['q_no']."';";
					$result2=mysqli_query($conn,$query2);
					while($row2=mysqli_fetch_array($result2)){
						//getting the tire details
...
?>
         	...
         	<div class="box-footer">
            	<button data-toggle="collapse" class="btn btn-sm btn-success" data-target="#<?php echo $row['q_no']?>">Make an order</button>
            	</br></br>
            	<!-- this div is hidden until click on Make an order-->
...
var discount=0;
var netamount=0;
function updatedata(){//update order footer
	$('#subtotal').val(subtotal);
	$('#Discount').val(discount);
	$('#netamount').val(netamount);
	
}	
$('#addall').change(function(){//add all items to order
		if(this.checked){
...
function  additem(element){// this function will run when the checkbox of each item changed
...
$('#placeorder').click(function(){
	...
	if(subtotal==0){//check if atlest one item is added to the order
